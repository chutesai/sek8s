...
3. **Principle of least privilege**
   - The systemd unit runs as an `inspector` user with membership in the `systemd-journal` and `video` groups. It does not require root and is fully confined via a drop-in (`ProtectSystem=strict`, `NoNewPrivileges=true`, etc.).
