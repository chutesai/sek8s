...
- PCI device ID `2bb1` is confirmed for the Workstation Edition only. Server Edition ID must be validated on hardware and added to `pci_device_ids` before server deployment.
...
|------|---------------|-----------------|
| Server Edition PCI device ID | Unknown (only Workstation `2bb1` confirmed) | Run `lspci -nn -d 10de:` on Server Edition hardware; add ID to `pci_device_ids` |
