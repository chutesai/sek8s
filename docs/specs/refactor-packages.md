...
- **Makefiles:** Package-scoped targets where useful (e.g. chutes-miner [`makefiles/`](https://github.com/chutesai/chutes-miner/tree/main/makefiles))—pragmatic, not parity for its own sake.
...
4. **Single** combined workflow vs **split** jobs for **version bump check** vs **pyproject sync commit** (chutes-miner splits `package-version-check` vs `pyproject-version-check`).
