...
- **Chute log shipper service (guest image, Phase 1).** New `chute-log-shipper` systemd service +
  ...
  - `sek8s.chute-log-shipper` AppArmor profile delivered via `apparmor-hardening`, confining the
    service to the chute log paths, the registry-tls leaf, the checkpoint dir, and egress to the
    validator. The CRI socket is **not** reachable from the service profile: pod discovery execs
    `crictl-pods-helper`, which transitions into a `crictl` child profile that holds the socket
    and nothing else, so a compromised shipper cannot drive the container runtime.
...
- Pinned guest HWE kernel bumped from `6.17.0-35.35~24.04.1` to `7.0.0-31.31~24.04.1`. Moves
  the RTMR/measurement baseline, so expected measurements must be regenerated.
...
- `base-image.yml`'s header no longer claims a third party can rebuild the layer and compare hashes.
  ...
  build-time ephemera, and an identical package set at identical versions. `base_image_sha256`
  proves a consumer received our artifact; the test of a correct rebuild is the guest measurements,
  not the layer hash. `docs/reproducing-measurements.md` says the same, so nobody reports a hash
  mismatch as a bug.
...
- `base-image.yml` no longer honours a cached build disk. `run-vm` only copies the prepared image
  over `/var/lib/libvirt/images/tdx-guest.qcow2` when the file is missing or `NO_CACHE=true`, so a
  leftover disk plus a forgotten flag meant the build booted whatever a previous run left behind and
  captured that as the layer — observed producing a base image containing `cuda-toolkit`, from a
  half-finished guest build, caught only because it was 3x the expected size. The playbook now sets
  `no_cache` itself: its output is a published artifact others pin by hash, and its inputs are
  hash-pinned, so a re-fetch costs time and never correctness.
