...
# Usage: OVERLAY=$(./prepare-vm-image.sh "$BASE_IMAGE" "$HOSTNAME" "$EXPECTED_BASE_SHA256" "$OVERLAY_DIR" [skip_checksum])
...
if [[ "$SKIP_VERIFY" == "1" || "$SKIP_VERIFY" == "true" || "$SKIP_VERIFY" == "yes" ]]; then
  ...
  SHA_FOR_OVERLAY="$EXPECTED_SHA"
fi

[[ -d "$OVERLAY_DIR" ]] || sudo mkdir -p "$OVERLAY_DIR"

OVERLAY_IMAGE="${OVERLAY_DIR}/tdx-${HOSTNAME}-${SHA_FOR_OVERLAY:0:16}.qcow2"
