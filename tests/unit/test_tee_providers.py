...
from sek8s.providers import tee as tee_module
from sek8s.providers.base import REPORT_DATA_BYTES
from sek8s.providers.snp import SNP_REPORT_SIZE, SnpQuoteProvider
from sek8s.providers.tdx import TdxQuoteProvider
from sek8s.providers.tee import TeeType, detect_tee, get_quote_provider


@pytest.fixture
def no_tee_devices(monkeypatch):
    """Neither guest device node exists."""
    monkeypatch.setattr(tee_module.os.path, "exists", lambda _p: False)
...
def test_detect_tee_prefers_tdx_device(monkeypatch):
    monkeypatch.setattr(
        tee_module.os.path, "exists", _only(tee_module.TDX_GUEST_DEVICE)
    )
    assert detect_tee() is TeeType.TDX


def test_detect_tee_finds_snp_device(monkeypatch):
    monkeypatch.setattr(
        tee_module.os.path, "exists", _only(tee_module.SEV_GUEST_DEVICE)
    )
    assert detect_tee() is TeeType.SNP


def test_detect_tee_raises_when_no_device(no_tee_devices):
...
@pytest.mark.parametrize(
    "override,expected",
    [("tdx", TeeType.TDX), ("snp", TeeType.SNP), (TeeType.SNP, TeeType.SNP)],
)
def test_detect_tee_override_skips_detection(no_tee_devices, override, expected):
    assert detect_tee(override) is expected


def test_get_quote_provider_returns_platform_provider(no_tee_devices):
    assert isinstance(get_quote_provider("tdx"), TdxQuoteProvider)
    assert isinstance(get_quote_provider("snp"), SnpQuoteProvider)
