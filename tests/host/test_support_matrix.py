...
def test_2510_no_longer_validated():
    # 26.04 is the only validated host OS; 25.10 hosts must upgrade first.
    assert not is_validated_topology("25.10", "H200", 8)
    assert not is_validated_topology("25.10", "B200", 8)
    assert not is_validated_topology("25.10", "RTX_PRO_6000", 8)
...
def test_wrong_gpu_count_not_validated():
    assert not is_validated_topology("26.04", "H200", 4)
